...
def __reset_vars() -> None:
    """Clear variables that store pointers to references to classes."""
    global __dynamic_panel_categories, __dynamic_panels, __panels
    __dynamic_panels = set({})
    __dynamic_panels = []
    __panels = (OBJECT_PT_ab_attributes,
                DATA_PT_ab_attributes,
                DATA_PT_ab_colors,
                DATA_PT_ab_uv_utilities)
...
-> list[bpy.types.Panel]:
    ...
    if not __init:
        ...
        RuntimeError(f"{ab_constants.panels_initialized}")
...
-> list[bpy.types.Panel]:
    ...
    if not __init:
        ...
        return __load(classes)
    else:
        RuntimeError(f"{ab_constants.error}{ab_constants.panels_initialized}")
...
def unload() -> None:
    ...
    if __init:
        ...
        RuntimeError(f"{ab_constants.error}{ab_constants.panels_not_initialized}")