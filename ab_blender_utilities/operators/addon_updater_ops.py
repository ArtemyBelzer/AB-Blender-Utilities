...
def get_user_preferences(context=None):
    ...
    if prefs:
        ...
        try:
            return ab_persistent.get_preferences()
        except:
            return None
...
# -----------------------------------------------------------------------------
def update_notice_box_ui(self, context, is_pie : bool):
    ...
    if not ab_debug.simulate_update_presence:
        if "ignore" in updater.json and updater.json["ignore"]:
            return
...
def register(bl_info):
    ...
    updater.subfolder_path = "ab_blender_utilities"
    ...
    updater.overwrite_patterns = ["*.png", "*.jpg", "README.md", "LICENSE.md"]
